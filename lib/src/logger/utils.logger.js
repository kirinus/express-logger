...
function createExpressWinstonHandler(logger, options) {
    return expressWinston.logger(Object.assign({ winstonInstance: logger, meta: true, metaField: 'express', msg: '{{req.method}} {{req.url}}', expressFormat: false, colorize: config_1.env.ENVIRONMENT === 'development', requestFilter: sanitizeHeaders, headerBlacklist: ['cookie', 'portal-authentication-token', 'token'], ignoreRoute: () => false }, options));
}
