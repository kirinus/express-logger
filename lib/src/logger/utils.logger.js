...
function sanitizeResponse(res, propName, options) {
    if (propName === 'body') {
        if (res['body'] && (options === null || options === void 0 ? void 0 : options.bodyBlacklist)) {
            console.log(res['body']);
        }
    }
    return res[propName];
}
function createExpressWinstonHandler(logger, options) {
    return expressWinston.logger(Object.assign({ winstonInstance: logger, meta: true, metaField: 'express', msg: '{{req.method}} {{req.url}}', expressFormat: false, colorize: config_1.env.ENVIRONMENT === 'development', requestFilter: sanitizeRequest, responseFilter: (res, propName) => sanitizeResponse(res, propName, options), headerBlacklist: ['cookie', 'portal-authentication-token', 'token'], ignoreRoute: () => false }, options));
}
