    ...
    describe('create winston logger', () => {
      ...
      `;
        ...
        describe('formatLogstash', () => {
            test('with string', () => {
                const expectedPrintfFormat = { "level": "info", "message": "{\"@fields\":{\"level\":\"info\"},\"@message\":\"string-test\",\"@timestamp\":\"2021-05-18T19:32:31.495Z\"}" };
                const formatter = src_1.formatLogstash();
                expect(formatter.transform({ level: 'info', message: 'string-test', timestamp: '2021-05-18T19:32:31.495Z' })).toMatchObject(expectedPrintfFormat);
            });
            test('with object', () => {
                const expectedPrintfFormat = { "level": "info", "message": "{\"@fields\":{\"level\":\"info\"},\"@message\":\"{\\\"description\\\":\\\"unit-test\\\"}\"}" };
                const formatter = src_1.formatLogstash();
                expect(formatter.transform({ level: 'info', message: { description: 'unit-test' } })).toMatchObject(expectedPrintfFormat);
            });
            test('without a message', () => {
                const expectedPrintfFormat = { "level": "info", "message": "{\"@fields\":{\"level\":\"info\"}}" };
                const formatter = src_1.formatLogstash();
                expect(formatter.transform({ level: 'info', message: undefined })).toMatchObject(expectedPrintfFormat);
            });
        });
